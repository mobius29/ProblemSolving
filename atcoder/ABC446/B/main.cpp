...
Vec<int> solve(const int &M, const Vec2<int> wishlists) {
  Vec<bool> juices(M + 1, false);
...
int main() {
  ...
  auto [N, M] = read<pii>(); // 1-index
  Vec2<int> wishlists(N);
  for (auto &wishlist : wishlists) {
    wishlist.assign(read<int>(), 0);
    for (auto &wish : wishlist) wish = read<int>();
  }

  auto answer = solve(M, wishlists);
  for (auto &el : answer) { cout << el << endl; }
