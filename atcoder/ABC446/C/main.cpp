...
int solve(const int &N, const int &D, const Vec<int> &A, const Vec<int> &B) {
  deque<pii> dq;
  for (int i = 0; i < N; i++) {
    dq.push_back({ A[i], i });
    ...
    int needed = B[i];
    while (needed > 0) {
      auto [eggs, day] = dq.front();
      dq.pop_front();

      needed -= eggs;
      if (needed < 0) dq.push_front({ -needed, day });
    }
...
int main() {
  ...
  // sum of N over all test cases is at most 200,000

  auto T = read<int>(); // 1 <= T <= 200,000
  while (T--) {
    auto [N, D] = read<pii>(); // 1 <= D <= N <= 200,000
    auto A = read<int>(N);
    auto B = read<int>(N);

    cout << solve(N, D, A, B) << endl;
  }
