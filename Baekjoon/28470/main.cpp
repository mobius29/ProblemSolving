...
ll solve(const int &N, const Vec<ll> &increase, const Vec<ll> &decrease, const Vec<double> &changes) {
  ll adrenaline = 0;

  for (int i = 0; i < N; i++) {
    ll k = round(changes[i] * 10);
    adrenaline += max(increase[i] * k / 10 - decrease[i], increase[i] - decrease[i] * k / 10);
  }
...
int main() {
  ...
  auto N = read<int>();
  auto increase = read<ll>(N);
  auto decrease = read<ll>(N);
  auto changes = read<double>(N);

  cout << solve(N, increase, decrease, changes) << endl;
