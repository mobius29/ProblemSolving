...
bool solve(const int M, const Vec2<int> &switches) {
  Vec<int> appear_count(M);
  for (const auto &sw : switches) {
    for (const auto &on : sw) { appear_count[on]++; }
  }

  for (const auto &sw : switches) {
    if (all_of(all(sw), [&appear_count](const int &idx) { return appear_count[idx] > 1; })) { return true; }
  }
...
int main() {
  ...
  Vec2<int> switches(N);
  for (int i = 0; i < N; i++) {
    switches[i].resize(read<int>());
    for (auto &el : switches[i]) { el = read<int>() - 1; }
  }

  cout << (solve(M, switches) ? 1 : 0) << endl;
