...
Vec<T> read(int size);
template <typename T>
Vec2<T> read(int r_size, int c_size);
...
optional<Vec<int>> solve(const int& N, const int& K) {
  if (K != 1) {
    Vec<int> arr(N, 0);
    iota(all(arr), 1);
    return arr;
  }
  if (N == 2 || N == 3) return nullopt;

  Vec<int> ret;
  ret.reserve(N);

  for (int i = (N % 2 ? N : N - 1); i >= 1; i -= 2) ret.push_back(i);
  for (int i = (N % 2 ? N - 1 : N); i >= 2; i -= 2) ret.push_back(i);
  return ret;
}
...
int main() {
  ...
  while (T--) {
    ...
    auto result = solve(N, K).value_or(Vec<int>(1, -1));
    for (const auto& el : result) { cout << el << ends; }
    cout << endl;
  }
...
T _read_tuple_like(index_sequence<Is...>) {
  T ret{};
