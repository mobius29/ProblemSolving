...
int solve(const int N, const Vec<pii> strings) {
  Vec<int> set_list;
  Vec<int> each_list;
  for (auto &[set_price, each_price] : strings) {
    set_list.push_back(set_price);
    each_list.push_back(each_price);
  }

  sort(all(set_list), [](const int a, const int b) { return a < b; });
  sort(all(each_list), [](const int a, const int b) { return a < b; });

  if (set_list[0] > 6 * each_list[0]) { return each_list[0] * N; }
  return set_list[0] * (N / 6) + min(set_list[0], each_list[0] * (N % 6));
}
