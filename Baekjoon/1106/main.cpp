...
int solve(const int &C, const int &N, const Vec<pii> &inputs) {
  Vec<int> dp(C + 1, INF);

  dp[0] = 0;
  for (int i = 1; i <= C; i++) {
    for (const auto &[cost, reward] : inputs) {
      int prev_reward = max(0, i - reward);
      dp[i] = min(dp[i], dp[prev_reward] + cost);
    }
  }
...
int main() {
  ...
  auto [C, N] = read<pii>();
  auto inputs = read<pii>(N);

  cout << solve(C, N, inputs) << endl;
